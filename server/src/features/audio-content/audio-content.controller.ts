...
import type { FastifyRequestWithValidatedBody } from '../../types/request.types';
...
import {
  ...
  getAudioContent,
  insertAudioContent,
} from '../../utils/queries/audio_content';
import { getAudioUrlFromStorage, uploadAudioToStorage } from '../../utils/queries/audio-storage';
import { getVoiceByUserId } from '../../utils/queries/voice';
import type { GenerateAudioInput } from './audio-content.schema';

export const getAudioContentController = async (
...
export const deleteAudioContentController = async (
  request: FastifyRequestWithValidatedBody<{ id: string }>,
  reply: FastifyReply
) => {
  try {
