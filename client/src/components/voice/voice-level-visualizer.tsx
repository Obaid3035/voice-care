...
const BAR_COUNT = 20;
// Thresholds optimized for clear conversational voice recording
const SILENCE_THRESHOLD = 0.05; // 5% - too quiet for clear voice capture
const GOOD_LEVEL_THRESHOLD = 0.15; // 15% - clear, audible conversational voice
...
export function VoiceLevelVisualizer({ isRecording, isPaused, stream }: VoiceLevelVisualizerProps) {
  ...
  const updateAudioLevels = () => {
    ...
    // Add some smoothing to prevent rapid fluctuations
    const smoothedLevel = Math.max(0, averageLevel - 0.02); // Remove very low background noise
    
    // Debug logging (remove in production)
    if (isRecording && !isPaused && smoothedLevel > 0.1) {
      console.log('Audio level:', {
        raw: averageLevel.toFixed(3),
        smoothed: smoothedLevel.toFixed(3),
        isSilent: smoothedLevel < SILENCE_THRESHOLD,
        isGood: smoothedLevel >= GOOD_LEVEL_THRESHOLD
      });
    }
    
    // Generate levels based on frequency data (more realistic visualization)
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-700 mb-6">
      ...
      <div className="flex items-end justify-center gap-1 h-24 mb-4">
        {audioLevels.levels.map((level, index) => {
          const getBarColor = () => {
          ...
          return (
            <div
              ...
              style={{
                ...
                transform: isRecording && !isPaused ? `scaleY(${0.8 + level * 0.4})` : 'scaleY(0.3)',
              }}
            />
          );
        })}
